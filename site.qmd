...
The networks used as examples in this section were inferred using the DIANE (@DIANE_InferringAnalyzingGeneRegulatory) and TDCor (@TDCOR_ThesisJulienLavenus) methods, which are two different approaches. The dataset used for inference is the LR dataset, which contains microarray analyses of root bending samples taken every three hours from 6 to 54 hours after gravistimulation (@LR). These data contain log2 expression values for all Arabidopsis genes detected on the microarray. Four independent biological replicates were performed at each time point to ensure the robustness and reliability of the data.
